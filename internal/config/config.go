...
func (c *Config) Validate() error {
	...
	if c.CollectionInterval < 10*time.Second {
		return fmt.Errorf("collection_interval must be at least 10 seconds, got %v", c.CollectionInterval)
	}

	if c.CollectionInterval > 5*time.Minute {
		return fmt.Errorf("collection_interval cannot exceed 5 minutes, got %v", c.CollectionInterval)
	}
...
func LoadConfig(filename string) (*Config, error) {
	...
	for scanner.Scan() {
		...
		case "collection_interval":
			if duration, err := time.ParseDuration(value); err == nil {
				config.CollectionInterval = duration
			}
		case "log_level":
		...
		case "container_stats_interval":
			if duration, err := time.ParseDuration(value); err == nil {
				config.Container.StatsInterval = duration
			}
		...
		case "vm_stats_interval":
			if duration, err := time.ParseDuration(value); err == nil {
				config.VM.StatsInterval = duration
			}
		case "libvirt_uri":
	...
	if config.ServerURL == "" {
		config.ServerURL = "https://api.nodewarden.com/agent"
	}
	if config.CollectionInterval == 0 {
		config.CollectionInterval = 120 * time.Second
	}
	if config.LogLevel == "" {
	...
	if config.Container.StatsInterval == 0 {
		config.Container.StatsInterval = 30 * time.Second
	}
	...
	if config.Process.ConfigFetchInterval == 0 {
		config.Process.ConfigFetchInterval = 5 * time.Minute
	}
