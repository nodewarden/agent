...
func NewHTTPTransmitter(cfg *config.Config, logger *slog.Logger) (MetricTransmitter, error) {
	if cfg == nil {
...
func (t *HTTPTransmitter) sendRequest(ctx context.Context, jsonData []byte) error {
	// Create request - append /data to the server URL for metrics endpoint
	endpoint := t.config.ServerURL + "/data"
...
func (t *HTTPTransmitter) createPayload(metrics []metrics.Metric) map[string]interface{} {
	...
	for i, metric := range metrics {
		// Extract hostname from labels if present
		hostname := metric.Labels["host"]
		if hostname == "" {
			hostname = "unknown"
		}
	...
	// Get first metric's hostname for agent info
	hostname := ""
	if len(metrics) > 0 && metrics[0].Labels != nil {
		hostname = metrics[0].Labels["host"]
	}
